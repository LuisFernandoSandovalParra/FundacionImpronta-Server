...
const createUser = async (req, res) => {
    const { name, last_name, birthdate, document_type, document_number, phone_number, email } = req.body;
    try {
        const user = await userService.createNewUser({ name, last_name, birthdate, document_type, document_number, phone_number, email });
...
const updateUser = async (req, res) => {
    ...
    try {
        const user = await userService.updateUserData(document_number, { name: name, last_name: last_name, phone_number: phone_number, email: email });
        res.status(200).json({ ok: true, info: user });
    } catch (error) {
        res.status(500).json({ ok: false, info: error });
    }
